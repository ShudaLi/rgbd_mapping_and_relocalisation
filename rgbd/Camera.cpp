...
void SCamera::generateMapXY4Undistort()
{
	float s = float(cv::sum( abs(_cvmDistCoeffs) )[0]); //convert scalar to float
	if ( s < 0.0005f )	{
		_bIsUndistortionOn = false;
		return;
	}
	...
	//cout << "SCamera::generateMapXY4Undistort()" << endl;
	return;
}
void SCamera::gpuUndistort( const GpuMat& original_img_, GpuMat* ptr_undistorted_ ) const{
	...
	{
		//INTER_NEAREST, the undistorted image will have some artifacts
		//INTER_LINEAR, the depth value will have some noises
		cuda::remap(original_img_, *ptr_undistorted_, _cvgmMapX, _cvgmMapY, INTER_NEAREST, BORDER_CONSTANT  );
	}
	return; 
}
...
{
	GLuint uTesture_;
	if (bRenderTexture_) uTesture_ = pGL_->gpuMapRgb2PixelBufferObj(gpu_img_);
